...
function RecenterMap({ activeSchool }) {
  ...
  useEffect(() => {
    // Tvinga kartan att fatta sin storlek direkt
    map.invalidateSize();

    // Kör en extra koll efter 500ms när Next.js har byggt klart DOM:en
    const timer = setTimeout(() => {
      map.invalidateSize();
    }, 500);

    if (!activeSchool || !activeSchool.lat || !activeSchool.lng) {
